function draw_coribs_y(coribs,style,skeleton,shape)
...
switch style
    ...
    case 'rainbow' % color coded by axis
        for i=1:length(shape);%size(coribs,1)
             pt_id = find(coribs(:,1)==i);
             pt_id = pt_id(1);
            c = coribs(pt_id,1);
            a = coribs(pt_id,2);
            v = coribs(pt_id,3);
...
%             c = [shape; skeleton_point; shape_point];
%             [x0,y0,segments]=selfintersect(c(:,1),c(:,2));
%    
%             shape_intersect = any(x0)
        end;
